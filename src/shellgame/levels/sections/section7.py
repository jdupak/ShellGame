...
class Level7_0(Level):
    """Level 7.0: Section 7 Introduction."""
...
class Level7_1(Level):
    """Level 7.1: Find Executable."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instructions="""# Level 7.1: Hledání spustitelného souboru
...
`shellgame submit -f script.sh`""",
    ...
    def validate(self, answer: Optional[str], state: Any) -> tuple[bool, str]:
        ...
        validator = StringValidator("script.sh")
        return validator.validate(answer, state.workspace)


class Level7_2(Level):
    """Level 7.2: Make Executable."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instructions="""# Level 7.2: Nastavení spustitelnosti
...
`shellgame submit -f run_me.sh`""",
    ...
    def validate(self, answer: Optional[str], state: Any) -> tuple[bool, str]:
        ...
        str_val = StringValidator("run_me.sh")
        success, msg = str_val.validate(answer, state.workspace)
        if not success:
            return False, msg

        validator = ExecutableValidator("level-7/permissions/run_me.sh")
        return validator.validate(answer, state.workspace)


class Level7_3(Level):
    """Level 7.3: Read Only."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instructions="""# Level 7.3: Ochrana souboru
...
`shellgame submit -f config.readonly`""",
    ...
    def validate(self, answer: Optional[str], state: Any) -> tuple[bool, str]:
        ...
        str_val = StringValidator("config.readonly")
        success, msg = str_val.validate(answer, state.workspace)
        if not success:
            return False, msg

        # Check if write permission is removed for user
...
class Level7_4(Level):
    """Level 7.4: Numeric Mode."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            section=7,
            title="Číselný zápis",
            instructions="""# Level 7.4: Číselný zápis
...
`shellgame submit -f public_html`""",
    ...
    def validate(self, answer: Optional[str], state: Any) -> tuple[bool, str]:
        ...
        str_val = StringValidator("public_html")
        success, msg = str_val.validate(answer, state.workspace)
        if not success:
            return False, msg

        validator = PermissionValidator("level-7/permissions/public_html", "755")
        return validator.validate(answer, state.workspace)


class Level7_5(Level):
    """Level 7.5: Section 7 Summary."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            instructions="""### 🎯 Výzva: Strážce oprávnění
...
`shellgame submit guardian`""",
    ...
    def validate(self, answer: Optional[str], state: Any) -> tuple[bool, str]:
        ...
        if answer.strip().lower() != "guardian":
            return False, "Heslo není správné. Splňte úkoly a odevzdejte: guardian"

        challenge_dir = state.workspace / "level-7" / "challenge"

        # Check script.sh - must be executable by owner
        script = challenge_dir / "script.sh"
        if not (script.stat().st_mode & stat.S_IXUSR):
            return False, "script.sh není spustitelný. Použijte 'chmod +x script.sh'."

        # Check secret.txt - no write permission for anyone
        secret = challenge_dir / "secret.txt"
        ...
        if actual != 0o644:
            return (
                False,
                f"shared.txt nemá oprávnění 644. Aktuální: {oct(actual)}. Použijte 'chmod 644 shared.txt'.",
            )
...
def get_levels() -> list:
    """
    Return all Section 7 level classes.

    Returns:
        List of Level instances for Section 7
    """
    return [
        Level7_0(),
        Level7_1(),
