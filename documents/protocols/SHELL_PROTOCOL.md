...
This document focuses on the **protocol contract** (markers + execution). For the “how it’s launched” details (bash/fish flags, greeting suppression), see `ARCHITECTURE.md`.
...
- **Fish**: greeting is suppressed for that session by overriding `fish_greeting`, then the integration script is sourced.
- **Bash**: user startup scripts are not sourced (`--noprofile --norc -i --rcfile <tempfile>`), and the integration script is sourced from that rcfile.
...
The generated `shellgame` wrapper function in the subshell intercepts stderr and executes lines emitted by Python that start with:
